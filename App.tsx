...
import DeployerTerminal from './components/DeployerTerminal';
import { IconDashboard, IconAgents, IconCanvas, IconBlueprint, IconForge, IconSettings, IconTrophy, IconActivity, IconRocket } from './components/Icons';
      ...
      <WalletProvider wallets={wallets} autoConnect>
        <WalletModalProvider>
          {currentView === AppView.LANDING ? (
            <LandingPage onStart={() => setCurrentView(AppView.DASHBOARD)} />
          ) : (
            <div className="flex h-screen bg-nexus-dark text-white overflow-hidden font-sans selection:bg-nexus-accent selection:text-black">
              ...
              <div className="w-20 md:w-64 flex-shrink-0 border-r border-gray-800 flex flex-col bg-black z-20">
                <div className="h-20 flex items-center justify-center md:justify-start md:px-6 border-b border-gray-800">
                  <div 
                    ...
                    className="w-8 h-8 bg-nexus-accent rounded-sm rotate-45 flex items-center justify-center shadow-[0_0_15px_rgba(0,240,255,0.5)] cursor-pointer"
                  >
                    <div className="w-4 h-4 bg-black -rotate-45"></div>
                  </div>
                  <span 
                    onClick={() => setCurrentView(AppView.LANDING)}
                    className="hidden md:block ml-4 font-bold text-xl tracking-tighter cursor-pointer"
                  >
                    OMNI<span className="text-nexus-accent">NEXUS</span>
                  </span>
                </div>
                ...
                <nav className="flex-1 py-8 space-y-2">
                  ...
                  <NavItem view={AppView.DEPLOYER} label="Deployer Terminal" icon={IconRocket} />
              ...
              <div className="flex-1 flex flex-col relative overflow-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-nexus-dark to-black">
                ...
                <main className="flex-1 relative overflow-hidden">
                  ...
                  {currentView === AppView.DEPLOYER && <DeployerTerminal />}
